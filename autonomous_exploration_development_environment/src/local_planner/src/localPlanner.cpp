...
double obstacleHeightThre = 0.2; //障碍物高度临界值
double obstacleRegionThreVar = 0.01; //区域方差临界值
...
// std::vector<std::vector<VoxelData>> intensityValues(pathNum, std::vector<VoxelData>());
...
void terrainCloudHandler(const sensor_msgs::PointCloud2ConstPtr& terrainCloud2)
{
  if (useTerrainAnalysis) {
    ...
    for (int i = 0; i < terrainCloudSize; i++) {
      ...
      float dis = sqrt((pointX - vehicleX) * (pointX - vehicleX) + (pointY - vehicleY) * (pointY - vehicleY));
      if (dis < adjacentRange && (point.intensity > obstacleRegionThreVar || useCost)) {
        point.x = pointX;
        point.y = pointY;
        point.z = pointZ;
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("obstacleRegionThreVar",obstacleRegionThreVar);
  ...
  while (status) {
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      /*添加障碍物处理：这段代码处理添加的障碍物点云数据。它将障碍物点云的坐标从车辆坐标系转换为规划器坐标系，
      然后计算每个障碍物点与车辆的距离，如果距离小于指定的邻近范围（adjacentRange），则将该点加入到规划器
      的剪裁点云（plannerCloudCrop）中*/
      int addedObstaclesSize = addedObstacles->points.size();
      for (int i = 0; i < addedObstaclesSize; i++) {
      ...
      /*路径范围和相对目标距离计算：根据速度和参数配置，计算路径范围（pathRange）和相对目标
      距离（relativeGoalDis）。如果启用了自主模式（autonomyMode），则根据车辆和目标的坐
      标计算相对目标的位置和距离，并计算车辆朝向角度（joyDir）。*/
      ...
      /*路径比例计算：根据速度和参数配置，计算路径比例（pathScale）*/
...
/*路径规划循环：该代码段是一个路径规划的主要循环。*/
      std::vector<std::vector<VoxelData>> intensityValues(36*pathNum, std::vector<VoxelData>());
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        ...
        /*计算障碍物最小角度：计算障碍物的最小角度，分为顺时针（CW）和逆时针（CCW）两个方向。*/
        ...
        for (int i = 0; i < plannerCloudCropSize; i++) {
          ...
          if (dis < pathRange / pathScale && (dis <= (relativeGoalDis + goalClearRange) / pathScale || !pathCropByGoal) && checkObstacle) {
            /*遍历路径方向和旋转角度：遍历36个路径方向和旋转角度。对于每个方向，计算旋转后的坐标（x2和y2），
              并计算比例因子（scaleY）以进行网格体素化。
            然后根据坐标计算所在网格体素的索引（indX和indY），并检查是否在有效的网格范围内。*/
            for (int rotDir = 0; rotDir < 36; rotDir++) {
              ...
              if (indX >= 0 && indX < gridVoxelNumX && indY >= 0 && indY < gridVoxelNumY) {
                ...
                /*更新路径列表和路径惩罚列表：根据网格体素的索引，
                更新路径列表（clearPathList）和路径惩罚列表（pathPenaltyList）*/
                 for (int j = 0; j < blockedPathByVoxelNum; j++) //ind行，j列,correspondences[ind][j]第ind行j列的数，pathNum * rotDir + correspondences[ind][j] 代表path
                {
                  if (h > obstacleRegionThreVar || !useTerrainAnalysis) {
                    clearPathList[pathNum * rotDir + correspondences[ind][j]]++;   //correspondences[ind][j]中存放体素index和路径之间的索引关系
                  } else {
                    if (pathPenaltyList[pathNum * rotDir + correspondences[ind][j]] < h && h > groundHeightThre) {
                      pathPenaltyList[pathNum * rotDir + correspondences[ind][j]] = h;  //目前已知具体对应哪个路径，但未知
                    }
                  }
                  if(saveTXT){
                    VoxelData voxel;
                  ...
                  voxel.y = y;
                  voxel.intensity = h;
                  // Update intensityValues array
                  intensityValues[pathNum * rotDir + correspondences[ind][j]].push_back(voxel);
                  //写路径文件：
                  // float indPathVoxel = pathNum * rotDir + correspondences[ind][j];
          ...
          // intensityFile << std::endl;
          /*检查障碍物碰撞：在路径范围内，对每个剪裁点云进行进一步处理以检查是否与障碍物发生碰撞。*/
          if (dis < diameter / pathScale && (fabs(x) > vehicleLength / pathScale / 2.0 || fabs(y) > vehicleWidth / pathScale / 2.0) && 
              (h > obstacleRegionThreVar || !useTerrainAnalysis) && checkRotObstacle) {
        ...
        if (saveTXT)
        {
          ...
          if(intensityFile.is_open()){
            for (const auto& path : intensityValues) 
          {
            for (const auto& voxel : path) 
            {
              intensityFile << voxel.x << " " << voxel.y << " " << voxel.intensity << "\n";
            }
            // 在路径之间添加一个空行
            intensityFile << "\n";
          }
