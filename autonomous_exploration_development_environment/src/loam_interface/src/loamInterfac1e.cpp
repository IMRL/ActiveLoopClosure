...
string PgoPathTopic = "/aft_pgo_path";
...
// void PgoPathHandler(const nav_msgs::Path::ConstPtr& path_msg)
// {
//     if (!path_msg->poses.empty())
//     {

//         latest_odometry.header = path_msg->header;
//         latest_odometry.pose.pose = path_msg->poses.back().pose;
//     }
//     double roll, pitch, yaw;
//     geometry_msgs::Quaternion geoQuat = latest_odometry.pose.pose.orientation;
//     odomData = latest_odometry;
...
//       odomData.pose.pose.orientation = geoQuat;
//       odomData.pose.pose.position.x = latest_odometry.pose.pose.position.z;
//       odomData.pose.pose.position.y = latest_odometry.pose.pose.position.x;
//       odomData.pose.pose.position.z = latest_odometry.pose.pose.position.y;
//     }
...
//     // publish tf messages
//     odomTrans.stamp_ = latest_odometry.header.stamp;
//     odomTrans.frame_id_ = "map";
//     odomTrans.child_frame_id_ = "sensor";
//     odomTrans.setRotation(tf::Quaternion(geoQuat.x, geoQuat.y, geoQuat.z, geoQuat.w));
//     odomTrans.setOrigin(tf::Vector3(odomData.pose.pose.position.x, odomData.pose.pose.position.y, odomData.pose.pose.position.z));
...
//         tfBroadcasterPointer->sendTransform(odomTrans);
//       } else {
//         tfBroadcasterPointer->sendTransform(tf::StampedTransform(odomTrans.inverse(), latest_odometry.header.stamp, "sensor", "map"));
//       }
//     }
// }
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subPath = nh.subscribe<nav_msgs::Path> (PgoPathTopic,5,PgoPathHandler);
