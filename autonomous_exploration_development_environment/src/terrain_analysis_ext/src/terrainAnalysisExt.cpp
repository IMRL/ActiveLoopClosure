...
const int planarVoxelWidth = 202;
...
pcl::PointCloud<pcl::PointXYZINormal>::Ptr terrainCloudElev(new pcl::PointCloud<pcl::PointXYZINormal>());
pcl::PointCloud<pcl::PointXYZINormal>::Ptr terrainCloudLocal(new pcl::PointCloud<pcl::PointXYZINormal>());
...
vector<float> planarVoxelPCA[planarVoxelNum];
  ...
  while (status)
  {
    ...
    if (newlaserCloud)
    {
      ...
      while (vehicleX - terrainVoxelCenX > terrainVoxelSize)
      {
        for (int indY = 0; indY < terrainVoxelWidth; indY++)
        {
          pcl::PointCloud<pcl::PointXYZINormal>::Ptr terrainVoxelCloudPtr = terrainVoxelCloud[indY];
      ...
      while (vehicleY - terrainVoxelCenY > terrainVoxelSize)
      {
        for (int indX = 0; indX < terrainVoxelWidth; indX++)
        {
          pcl::PointCloud<pcl::PointXYZINormal>::Ptr terrainVoxelCloudPtr = terrainVoxelCloud[terrainVoxelWidth * indX];
      ...
      //for (int ind = 0;ind<terrainVoxelNum;ind++)
      // {
      //   if(!terrainVoxelCloud[ind]->empty()){
      // ROS_INFO("terrainVoxelCloud[%i] have normal_x:%f",ind,(*terrainVoxelCloud[ind])[1].normal_x);
      // }
      // }
          ...
          for (int dX = -1; dX <= 1; dX++)
          {
            for (int dY = -1; dY <= 1; dY++)
            {
              if (indX + dX >= 0 && indX + dX < planarVoxelWidth && indY + dY >= 0 && indY + dY < planarVoxelWidth)
              {
                ...
                planarVoxelPCA[planarVoxelWidth * (indX + dX) + indY + dY].push_back(point.normal_x) ;
              }
            }
          }
        }
      ...
      for (int i = 0; i < terrainCloudSize; i++)
      {
        ...
        if (point.z - vehicleZ > lowerBoundZ - disRatioZ * dis && point.z - vehicleZ < upperBoundZ + disRatioZ * dis && dis > localTerrainMapRadius)
        {
          ...
          if (indX >= 0 && indX < planarVoxelWidth && indY >= 0 && indY < planarVoxelWidth)
          {
            ...
            int planarVoxelPCASize = planarVoxelPCA[ind].size();
            if (disZ < vehicleHeight && (planarVoxelConn[ind] == 2 || !checkTerrainConn))
            {
              ...
              for(const auto& p: planarVoxelPCA[ind])
              {
                  sumpca +=p;
              }
              float smallestEigenvalue = sumpca/planarVoxelPCASize;
              
              for(const auto& p: planarPointElev[ind])
              {
                  sum +=p;
                  sumSquared += p*p;
              }
              ...
              terrainCloudElev->points[terrainCloudElevSize].curvature = point.curvature;
              terrainCloudElev->points[terrainCloudElevSize].normal_x = smallestEigenvalue;
              terrainCloudElev->points[terrainCloudElevSize].normal_y = std_dev;
      ...
      {
        pcl::PointXYZINormal point = terrainCloudLocal->points[i];
        float dis = sqrt((point.x - vehicleX) * (point.x - vehicleX) + (point.y - vehicleY) * (point.y - vehicleY));
